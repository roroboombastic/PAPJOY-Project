...
async function addCartItem(req, res) {
  try {
    ...
    if (availableStock < quantity) {
      return res.status(400).json({ success: false, error: `Only ${availableStock} items available` });
    }
...
async function updateCartItem(req, res) {
  try {
    ...
    if (!existing) return res.status(404).json({ success: false, error: 'Item not found in cart' });
